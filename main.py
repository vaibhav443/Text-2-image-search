...
def searched_images(search_query, directory, N_images):
    image_features, image_attributes = get_image_features(directory)
...
if __name__ == "__main__":
    params = {"directory": r"C:\Users\vaibh\Downloads\PM1 Internship\natural-language-joint-query-search\images",
              ...
              "N_images": 3}
    start = time.time()
    list_res = searched_images(params.get("search_query"), params.get("directory"), params.get("N_images", 3))
    end = time.time()
    print(end - start)
    print(list_res)
